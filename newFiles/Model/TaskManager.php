...
class TaskManager
{
    ...
    {
        try {
            $request = PdoFactory::getPdo()->prepare("SELECT id_task, title_task, content_task, achieved_task, id_priority FROM task WHERE id_todo = :id_todo");
            $request->execute(array(':id_todo' => $todoObject->getId()));
            while ($result = $request->fetch()) {
                foreach ($list_priority as $priority) {
                    if ($priority->getId() == $result["id_priority"]) {
                        $task = new Task($result["id_task"], $result["title_task"], $result["content_task"], $result["achieved_task"], $todoObject->getUserObject(), $todoObject, $priority);
                    }
                }
            }
        } catch (Exception $e) {
    ...
    {
        try {
            $request = PdoFactory::getPdo()->prepare("SELECT title_task, content_task, achieved_task FROM task WHERE id_task = :id_task");
            $request->execute(array(':id_task' => $idTask));
            $result = $request->fetch();
            $task = new Task($idTask, $result["title_task"], $result["content_task"], $result["achieved_task"], $userObject, $todoObject, $priorityObject);
        } catch (Exception $e) {
            throw new Exception($e);
        }
    }
     ...
     * @return void
     */
    public static function insertTask(string $title, string $content, Priority $priorityObject, Todo $todoObject, User $userObject)
    {
        try {
            $request = PdoFactory::getPdo()->prepare("call createTask (:p_idUser, :p_idTodo, :p_titleTask, :p_contentTask, :p_idPriority)");
            $request->execute(array(':p_idUser' => $userObject->getId(), ':p_idTodo' => $todoObject->getId(), ':p_titleTask' => $title, ':p_contentTask' => $content, ':p_idPriority' => $priorityObject->getId()));
