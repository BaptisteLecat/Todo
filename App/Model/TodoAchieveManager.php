...
class TaskAchieveManager
{
    ...
    public static function loadTaskAchieveFromTodo(Todo $todoObject)
    {
        try {
            ...
            FROM user, task_archive, taskachieve_date, todo, task 
            WHERE user.id_user = task_archive.id_user
            AND task_archive.id_date = taskachieve_date.id_achieve
            ...
            while ($result = $request->fetch()) {
                foreach ($todoObject->getList_Task() as $task) {
                    if ($task->getId() == $result["id_task"]) {
                        $taskUpdated = new TaskUpdated($result["date_achieve"], $result["name_user"], $task);
                        break;
                    }
                }
            }
    ...
    public static function loadTaskAchieveFromTask(Task $taskObject)
    {
        try {
            ...
            FROM task, task_archive, taskachieve_date, user 
            WHERE task.id_task = task_archive.id_task 
            AND taskachieve_date.id_achieve = task_archive.id_date 
            AND user.id_user = task_archive.id_user 
            AND task.id_task = :id_task");
            $request->execute(array(':id_task' => $taskObject->getId()));
            $result = $request->fetch();
            if ($request->rowCount() > 0) {
                $taskUpdated = new TaskUpdated($result["date_achieve"], $result["name_user"], $taskObject);
            }
        } catch (Exception $e) {
            throw new Exception($e);
        }
