            ...
            if($request->execute(array(':iduser_todo' => $userObject->getId()))){
                if($request->rowCount() > 0){
                    while ($result = $request->fetch()) {
                        $todo = new Todo(intval($result["id_todo"]), $result["title_todo"], $result["description_todo"], intval($result["active_todo"]), $result["status_todo"], $result["enddate_todo"], $result["endtime_todo"], $result["createdate_todo"], $userObject);
                    }
                }
                $response = ["success" => 1];
            }
